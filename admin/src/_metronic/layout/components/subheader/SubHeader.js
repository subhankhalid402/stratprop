...
export function SubHeader() {
  ...
  useLayoutEffect(() => {
    ...
    const aside_routes = getBreadcrumbsAndTitle("kt_aside_menu_routes", location.pathname);
    const aside_users = getBreadcrumbsAndTitle("kt_aside_menu_users", location.pathname);
    const aside_payments = getBreadcrumbsAndTitle("kt_aside_menu_payments", location.pathname);
    const header = getBreadcrumbsAndTitle("kt_header_menu", location.pathname);
    const breadcrumbs = ((aside && aside.breadcrumbs.length > 0) ? aside.breadcrumbs : header.breadcrumbs ) || 
    ((aside_routes && aside_routes.breadcrumbs.length > 0) ? aside_routes.breadcrumbs : header.breadcrumbs);
    subheader.setBreadcrumbs(breadcrumbs);
    subheader.setTitle(
      ((aside && aside.title && aside.title.length > 0) ? aside.title : header.title) ||
      ((aside_routes && aside_routes.title && aside_routes.title.length > 0) ? aside_routes.title : header.title)
    );
    ...
    >
      ...
      >
        ...
        {*/}
          ...
          <Link to="/logout" className={`btn btn-fixed-height btn-primary font-weight-bold px-2 px-lg-5 mr-2`}>
